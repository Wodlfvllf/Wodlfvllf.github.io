# Portfolio workspace

## Setup checklist

- [x] Requirements confirmed
- [x] Dependency-free static project scaffolded
- [x] Portfolio content and responsive design completed
- [x] No extensions required
- [x] HTML, CSS, and JavaScript diagnostics passed
- [x] No build or VS Code task required
- [x] Desktop and mobile previews validated
- [x] Hosting and editing instructions documented

## Project guidance

- Keep the site deployable as plain static files with no build step.
- Preserve the warm editorial palette; do not introduce blue or purple gradients.
- Keep résumé content in `index.html`, styling in `styles.css`, and interactions in `script.js`.
- Maintain responsive behavior, keyboard access, reduced-motion support, and relative asset paths.
